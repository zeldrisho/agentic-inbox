...
- Dependencies: `@oxlint/plugins` is supplied transitively by Vite+; anti-slop imports its plugin API from `vite-plus/lint/plugins`. `oxlint` is also supplied by Vite+ and was not installed separately.
